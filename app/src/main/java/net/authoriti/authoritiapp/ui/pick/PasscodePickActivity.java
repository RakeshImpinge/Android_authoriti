    ...
    void callAfterViewInjection() {
                ...
                if (pickerType.equals(PICKER_TIME) && position == picker.getValues().size() - 1) {
                    ...
                    if (picker.getPicker().equals(PICKER_DATA_TYPE)) {
                        List<Value> values = dataManager.getSelectedValuesForDataType(utils
                                .getPickerSelectedIndex(PasscodePickActivity.this, PICKER_REQUEST));
                        if (values == null) {
        ...
        if (picker.getValues() != null && picker.getValues().size() != 0) {
            ...
            if (picker.getPicker().equals(PICKER_DATA_TYPE)) {
                ...
                List<Value> values = dataManager.getValuesFromDataType(utils
    ...
    private boolean isSelected(Value value) {
        ...
        if (values != null && values.size() > 0) {
            for (Value value1 : values) {
                if (value1.getValue().equals(value.getValue()) && value1.getTitle().equals(value
                        .getTitle())) {
                    return true;
                }
            }
